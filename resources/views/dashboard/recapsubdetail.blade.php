...
<main class="w-full flex-grow p-6">
        <h1 class="text-sm font-bold"><a class="text-blue-500" href="{{ route('dashboard')}}">Dashboard</a>><a class="text-blue-500" href="{{ route('tax.recap')}}">Rekapitulasi Data SPPT Sudah Terbayarkan</a>><a class="text-blue-500" href="{{ route('tax.recapdetail', $date)}}">{{$date}}</a>>{{$name}}</h1>

        <div class="w-full mt-6">
            <div class="px-6 py-4 mb-3 overflow-hidden border rounded-lg shadow border-secondary-300 bg-white mx-1">
            ...
            <div class="flex flex-wrap mb-3">
                    ...
                    <div class="bg-white border rounded-lg shadow p-2">
                        <div class="flex flex-row items-center">
                            <div class="flex-shrink pr-4">
                                <div class="rounded p-3 bg-blue-600"><i class="fa fa-user fa-2x fa-fw fa-inverse"></i></div>
                            </div>
                            <div class="flex-1 text-right md:text-center">
                                <h5 class="font-bold uppercase text-gray-500">Petugas</h5>
                ...
                <div class="w-full md:w-1/2 xl:w-1/2 p-1">
                    ...
                    <div class="bg-white border rounded-lg shadow p-2">
                        <div class="flex flex-row items-center">
                            <div class="flex-shrink pr-4">
                                <div class="rounded p-3 bg-pink-600"><i class="fas fa-calendar-alt fa-2x fa-fw fa-inverse"></i></div>
                            </div>
                            <div class="flex-1 text-right md:text-center">
                                <h5 class="font-bold uppercase text-gray-500">Tanggal</h5>
            ...
            <div class="bg-white overflow-auto p-2 border rounded-lg shadow border-secondary-300 mx-1">
                <table class="text-left w-full border-collapse mb-2"> <!--Border collapse doesn't work on this site yet but it's available in newer tailwind versions -->
                    ...
                    @if ($pays->count() == 0)
                      ...
                      @else
                        @foreach ($pays as $pay)
                        <tr class="hover:bg-grey-lighter">
                          <td class="py-4 px-6 border-b border-grey-light ">{{$pay->tax->tax_number}}</td>
                          <td class="py-4 px-6 border-b border-grey-light ">
                                <p>{{$pay->tax->taxpayer_name}}</p>
                                <p class="text-xs">{{$pay->tax->taxpayer_address}}, {{$pay->tax->rt}}/{{$pay->tax->rw}}</p>
                          </td>
                          <td class="py-4 px-6 border-b border-grey-light ">{{$pay->tax->taxobject_address}}</td>
                          <td class="py-4 px-6 border-b border-grey-light ">@currency($pay->tax->value)</td>
                        <tr>
                        @endforeach
                      @endif
                    </tbody>
